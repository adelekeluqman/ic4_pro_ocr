...
for filename in os.listdir(filepath):   # Loop over all files in the working directory.
    ...
    # #Phone No infer_by_web(path, type)
    # # X,Y,W,H=325,980,750,70
    # X,Y,W,H=270,910,860,90
    # PhoneNoImage=Processed_Form[Y:Y+H,X:X+W]
    # # PhoneNoImage=Processed_Form
    # print("processed image to pass for handwritten : ", PhoneNoImage)
    # show_wait_destroy("processed", PhoneNoImage)    
    # PhoneNo=HandWrittenDigitRecognition(PhoneNoImage)
    # # PhoneNo=infer_by_web(PhoneNoImage, type)
