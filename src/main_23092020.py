...
from src.DataLoader import Batch, DataLoader, FilePaths
from src.SamplePreprocessor import preprocessor, wer
from src.Model import DecoderType, Model
from src.SpellChecker import correct_sentence
# from readtextfromimage import show_wait_destroy


####################################################################
#               show_wait_destroy() Function for Display           #
####################################################################
def show_wait_destroy2(winname, img):
...
def infer(model, fnImg):
    print("Reading received image for handwritten : ", fnImg);         # modified 08092020
    """ Recognize text in image provided by file path """
    # img = preprocessor(cv2.imread(fnImg, cv2.IMREAD_GRAYSCALE), imgSize=Model.imgSize)  # effect changes here
    # img = preprocessor([cv2.imread(fnImg, cv2.IMREAD_GRAYSCALE)], imgSize=Model.imgSize)  # effect changes here
    
    # img = cv2.imread(fnImg, cv2.IMREAD_GRAYSCALE)  # effect changes here
    
    # x3,y3,w3,h3=270,530,860,100
    # img = img[y3:y3+h3, x3:x3+w3]

    img = fnImg

    # show_wait_destroy2("received_cropped", img)
    
    if img is None:
        print("Image not found")

    # img = img.resize((800, 64))
    img = np.reshape(img, (860, 100))
    ...
    # imgs = np.append([img],imgs)  
    
    # imgs = imgs.reshape(1058800, 800, 64)
    # imgs = np.reshape(imgs, (21, 800, 64))
    
    # imgs = [img]  
    # imgs = imgs.reshape(1, imgs)
