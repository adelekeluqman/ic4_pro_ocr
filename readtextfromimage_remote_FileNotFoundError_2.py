...
with pysftp.Connection(host=myHostname, username=myUsername, password=myPassword, cnopts=cnopts) as sftp:
    ...
    for filename2 in sftp.listdir(micrpath):
        if (filename2.endswith('.png') or filename2.endswith('.jpg')): 
            ...
            micrfile = os.path.join(micrpath, filename2)
    ...
    # for attr in directory_structure:        
        ...
        # im = os.path.join(source, filename)
        print ('--------im is---' + im)
        ...
        # image = cv2.imread(im)
        # image = cv2.imread(sftp.open(im, mode='r', bufsize=-1))
        ...
        image = cv2.imdecode(np.fromfile(os.path.join(filepath + im), dtype=np.uint8), cv2.IMREAD_UNCHANGED)
