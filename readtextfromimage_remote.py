...
with pysftp.Connection(host=myHostname, username=myUsername, password=myPassword, cnopts=cnopts) as sftp:
    ...
    # for filename in directory_structure:        
    #     file = filename
    #     sftp.get(remote_dir + file, local_dir + file)
...
for filename in os.listdir(filepath):   # Loop over all files in the working directory.
    ...
    if not (filename.endswith('.png') or filename.endswith('.jpg')):    
        ...
        posts_ticket = db.ic4_ocr_ticket
        # posts_ex = db.ic4_ocr_ex_copy
        post_ticket_data = {  
            'ticket_date': datetime.now().date().strftime("%Y%d%m"),
            'ticket_name': filename 
        }
        result = posts_ticket.insert_one(post_ticket_data)
        # print('One rejected: {0}, {1}'.format(result.inserted_id, 'incompatible image format'))
