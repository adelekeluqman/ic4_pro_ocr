...
for filename in os.listdir(filepath):   # Loop over all files in the working directory.
    ...
    if out :
        if (out[0] == 'U') and (len(out) <= 6) :
            ...
            # }
            # import bank_check_ocr
            # python bank_check_ocr.py --image im --reference micrfile
            # from subprocess import call
            # call(["python", "bank_check_ocr.py --image im --reference micrfile"])
            # Popen('python bank_check_ocr.py --image im --reference micrfile')
